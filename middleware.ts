...
import { edgeRateLimit } from "@/lib/rate-limiter-edge";
...
export async function middleware(request: NextRequest) {
  ...
  const isPublicWriteRoute = path.startsWith("/api/contact");

  const response = supabaseResponse;

  if (isAIRoute || isPublicWriteRoute) {
    const { allowed, remaining } = await edgeRateLimit(request);
