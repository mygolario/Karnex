...
export function AccountSecurity({ bundle }: AccountSectionProps) {
  ...
  useEffect(() => {
    fetch("/api/user/security")
      .then((r) => r.json())
      .then((d: SecuritySummary) => setSecurity(d))
      .catch(() => {});
    fetch("/api/user/sessions")
      .then((r) => r.json())
      .then((d: { sessions: SessionListItem[]; loginEvents: LoginEventItem[] }) => {
  ...
  return (
    <div className="space-y-6">
      <AccountSectionHeader
        title="امنیت"
        subtitle="رمز عبور، نشست‌ها و تاریخچه ورود."
        icon={Shield}
        accent="primary"
      />
      ...
      <SettingsCard title="سلامت حساب" icon={ShieldCheck} accent="primary">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <HealthItem
            ok={(security?.connectedAccounts?.length || 0) > 0}
            label="حساب متصل (OAuth)"
          />
        </div>
      </SettingsCard>
      ...
      <SettingsCard title="رمز عبور و احراز هویت" icon={Lock} accent="primary">
        <SettingsRow
          ...
          description={
            passwordSet
              ? "یک رمز عبور تنظیم شده است"
              : "حساب از طریق OAuth وارد می‌شود"
          }
        >
          <Button
            variant="outline"
            size="sm"
            onClick={() =>
              toast.info("تغییر رمز از طریق صفحه ورود کارنکس انجام می‌شود")
            }
          >
            تغییر رمز
          </Button>
        ...
        <SettingsRow
          label="تأیید دو مرحله‌ای (2FA)"
          description="به‌زودی — فعلاً ورود امن از طریق ایمیل/گوگل کافی است"
          divider={false}
        >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-sm">
                      {s.device || "دستگاه ناشناخته"}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {s.location || s.ip || ""} • آخرین فعالیت:{" "}
                      {new Date(s.lastActive).toLocaleDateString("fa-IR")}
                    </div>
                  </div>
                </div>
                <Button
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium">
                    {e.status === "success" ? "ورود موفق" : "ورود ناموفق"}
                  </span>
                </div>
                <div className="text-xs text-muted-foreground">
